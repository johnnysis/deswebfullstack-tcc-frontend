...
const CreateEdit = (props) => {
    ...
    const save = () => {
        ...
        if(messages.length > 0)
            ...
            axios.post(`${url}/formasPagamento/create`, {codigo, descricao})
            .then(
                res => {
                    props.metodos();
    ...
    useEffect(() => {
        if(props.codigo && props.editando)
            axios.get(`${url}/formasPagamento/${props.codigo}`)
            .then(response => {
    ...
    return(
        <Form
            cardSize="col-sm-4"
            cardTitle={props.codigo && props.editando ? "Editar forma de pagamento" : "Nova forma de pagamento" }
            save={save}
